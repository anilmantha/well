...
<!-- changes -->
    ...
    <div class="container-fluid"> 
      ...
      <div class="row">
        <div class="col-lg-12">
          <h1 class="page-header"><small>Stock warningLevel Report</small> </h1>
          </div>
      </div>
      <!-- Main Content Area -->
      <div class="row">
        <div class="col-lg-12">
          <div class="panel panel-default">
            <div class="panel-heading">
              <h3 class="panel-title">List of warningLevels</h3>
            </div>
            <div class="panel-body">
              <div class="row">
                <div class="col-md-12">
                  <form class="form-inline" action="searchstockwarninglevel" method="post">
              ...
              <div class="row">
                <div class="table-responsive padding-10">
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                    <c:if test="${stockWarningList!=null}">
                    <c:forEach var="warningList" items="${stockWarningList}">
                      <tr id="trow">
                        <td>${warningList.getSno()}</td>
                        <td>${warningList.getStockname()}</td>
                        <td>${warningList.dropdowndetails.getDescription()}</td>
                          <td>${warningList.getAvailable()}</td>
                         <td>${warningList.getWarninglevel()}</td>
                      	<td>${warningList.getStocksubgroupmaster().getStockgroupmaster().getStockgroupname()}</td>
                        <td>${warningList.getStocksubgroupmaster().getStocksubgroupname()}</td>
						</tr>
                      </c:forEach>
                      </c:if>
                    <%--  <c:if test="${msg!=null}">
    ...
    $('#generateform').click(function(){
    	...
    	$('#reportform').attr('action','generateStockWarningLevelReport?stockname='+stockname+'&&stocktype='+stocktype+'&&stockgroup='+stockgroup+'&&stocksubgroup='+stocksubgroup+'&&type='+stocktype1+'&&group='+stockgroup1+'&&subgroup='+stocksubgroup1);
    	$('#reportform').submit();
    });
